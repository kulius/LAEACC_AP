Public Class AC050
    ...
    Private Sub AC050_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        LoadAfter = True
    ...
    Private Sub btnSure_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSure.Click
        ...
        myDatasetS = openmember(DNS_ACC, "acf010s", sqlstr)
        ...
        For intI = 0 To myDatasetS.Tables("acf010s").Rows.Count - 1
            ...
            sYear = Val(Mid(sdate, 1, 4)) - 1911 'Year(sdate) - 1911  because 2008/2/29 error
            If Mid(sdate, 5, 5) = "/2/29" Or Mid(sdate, 5, 6) = "/02/29" Then ' "2008/2/29" "2012/2/29"
                sMm = 2
            Else
                sMm = Month(sdate)
            End If
            ...
            '統計過帳張數
            If sItem = "1" And sSeq = "1" Then   '轉帳傳票
    ...
    Sub Upd_ACf070()   '產生日計檔
        ...
        sqlstr = "SELECT * from acf070 where date_2='" & sdate & "' and accno='" & sAccno & "'"
        mydataset = openmember(DNS_ACC, "acf070s", sqlstr)
        If mydataset.Tables("acf070s").Rows.Count = 0 Then   '原無該日餘額則新增一筆
            GenInsSql("date_2", sdate, "D")
            GenInsSql("accno", sAccno, "T")
            sqlstr = "insert into acf070 " & GenInsFunc
            retstr = runsql(mastconn, sqlstr)
            sqlstr = "SELECT * from acf070 where date_2='" & sdate & "' and accno='" & sAccno & "'"
            mydataset = openmember("", "acf070s", sqlstr)
        End If
        keyvalue = mydataset.Tables("acf070s").Rows(0).Item("autono")
        If sDC = "1" Then '借方  由acf010 update acf070,acf010->借方各依銀行存款 專戶存款 or 轉帳,分別加入各欄金額
            If sKind = "3" Then   '轉帳
                GenUpdsql("deamt3", mydataset.Tables("acf070s").Rows(0).Item("deamt3") + sAmt, "N")
            Else    '非轉帳時,要依該科目支出時是以銀行存款or專戶存款支出
                Select Case sBankAccno
                    Case "11102"
                        GenUpdsql("deamt1", mydataset.Tables("acf070s").Rows(0).Item("deamt1") + sAmt, "N")
                    Case "11103"
                        GenUpdsql("deamt2", mydataset.Tables("acf070s").Rows(0).Item("deamt2") + sAmt, "N")
                End Select
            End If
        Else             '貸方
            If sKind = "4" Then   '轉帳
                GenUpdsql("cramt3", mydataset.Tables("acf070s").Rows(0).Item("cramt3") + sAmt, "N")
            Else     '非轉帳時,要依該科目收入時是以銀行存款or專戶存款收入
                Select Case sBankAccno
                    Case "11102"
                        GenUpdsql("cramt1", mydataset.Tables("acf070s").Rows(0).Item("cramt1") + sAmt, "N")
                    Case "11103"
                        GenUpdsql("cramt2", mydataset.Tables("acf070s").Rows(0).Item("cramt2") + sAmt, "N")
                End Select
            End If
        End If

        sqlstr = "update acf070 set " & genupdfunc & " where autono=" & keyvalue
        retstr = runsql(mastconn, sqlstr)
        If retstr <> "sqlok" Then
            MsgBox("acf070 error sqlstr=" & sqlstr)
        End If
    End Sub
    ...
    Sub Upd_AMT()   '過帳
        ...
        If sItem = 1 Then  '表示有明細科目資料acf020待過帳
            ...
            If myDatasetT.Tables("acf020s").Rows.Count = 0 Then
                MsgBox("acf020沒有明細科目資料,請檢查kind=" & sKind & ",號數=" & sNo)
                Exit Sub
            End If
