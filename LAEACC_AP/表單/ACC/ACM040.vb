...
Public Class ACM040
    ...
    Private Sub ACM040_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub BtnPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnPrint.Click
        ...
        Dim document As New FPDocument("應 收 款 項 明 細 表")
        ...
        For PageCnt As Integer = 1 To 99    '頁次
            ...
            page.Add(textUnit)
            Dim textTitle As New FPText("應收款項明細表", 90, 6)
            ...
            Dim grid As New FPTable(0, 17, 255, 162, PageRow, 9)    'x,y,w,h,row,col
            grid.Font.Size = 10
            ...
            grid.ColumnStyles(2).Width = 65
            grid.ColumnStyles(3).Width = 27
            grid.ColumnStyles(4).Width = 26
            grid.ColumnStyles(5).Width = 30
            grid.ColumnStyles(6).Width = 22
            grid.ColumnStyles(7).Width = 30
            grid.ColumnStyles(8).Width = 22
            grid.ColumnStyles(9).Width = 25
            ...
            grid.Texts2D(1, 3).Text = "　應　收　數"
            grid.Texts2D(1, 4).Text = "收　　　　入　　　　數"
            grid.Texts2D(1, 6).Text = "減　　　　免　　　　數"
            grid.Texts2D(2, 4).Text = "本月實收數"
            grid.Texts2D(2, 5).Text = "本月累計實收數"
            grid.Texts2D(2, 6).Text = "本月減免數"
            grid.Texts2D(2, 7).Text = "本月累計減免數"
            grid.Texts2D(1, 8).Text = "轉入催收款"
            ...
            With myds.Tables("acm010")
                For i As Integer = 3 To PageRow
                    ...
                    grid.Texts2D(i, 8).Text = FormatNumber(nz(.Rows(intR).Item("amt6"), 0), 2)
                    grid.Texts2D(i, 9).Text = FormatNumber(nz(.Rows(intR).Item("amt7"), 0), 2)
                    intR += 1
                Next
            End With
    ...
    Private Sub btnExport_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnExport.Click
        ...
        Try
            ...
            Try
                tt1 = "c:\App\acc\報表樣本\ACM040.xls"
                tt2 = "c:\App\acc\報表\ACM040.xls"
                If Not File.Exists(tt1) Then
                    AppReport_Copy("acc", "ACM040.xls", tt1)
            ...
            Catch ex As Exception
                MsgBox("報表copy from " & tt1 & "  to  " & tt2 & " 錯誤，若 \報表\ACM040.XLS 使用中請先關閉之，否則請洽資訊人員!")
