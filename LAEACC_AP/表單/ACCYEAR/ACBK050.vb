...
Public Class ACBK050
    ...
    Private Sub ACBK050_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        '找過帳日期
        Dim sqlstr As String
    ...
    Private Sub BtnPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnPrint.Click
        ...
        Dim document As New FPDocument("列印收入明細分類帳 ")
        ...
        For intD = 0 To mydsD.Tables("DataAccno").Rows.Count - 1
            ...
            With mydataset.Tables(0).Rows(0)
                ...
                If Mid(strAccno, 8, 2).Trim <> "" And Len(strAccno) > 9 Then  '六級
                    ...
                    End If
                End If
                strName4 = nz(.Item("name4"), "") '列印科目
                '上月累計
                AcuAmt = nz(.Item("cramt"), 0) - nz(.Item("deamt"), 0)
                AcuAccount = 0
            ...
            For PageCnt As Integer = 1 To 999    '頁次
                ...
                If blnIsBelong Then
                    ...
                    textTitle = New FPText("收 入 明 細 分 類 帳", 90, 6)
                    ...
                    grid.Texts2D(1, 7).Text = "收           入            數            ."
                    grid.Texts2D(1, 4).Text = "　　　　　　　　摘　　　　　　　　　　　要"
                    grid.Texts2D(1, 5).Text = "預   算   數   ."
                    grid.Texts2D(1, 6).Text = "預 算 分 配 數 ."
                    grid.Texts2D(2, 7).Text = "實   收   數   ."
                    grid.Texts2D(2, 8).Text = "應   收   數 　."
                    grid.Texts2D(1, 9).Text = "未收入之分配數"
                Else
                    textUnit = ttextUnit
                ...
                With mydataset.Tables("acf020")
                    Do While i < PageRow
                        If intR > .Rows.Count - 1 Then
                            If MonBgtot + MonBGamt + MonAmt + MonAccount <> 0 Then
                                i += 1
                            ...
                            End If
                            If AcuBGtot + AcuBGamt + AcuAmt + AcuAccount <> 0 Then
                                i += 1
                                If i > PageRow Then Exit Do
                                strRemark = "　　　　本　　月　　累　　計"
                                grid.Texts2D(i, 4).Text = strRemark
                        ...
                        Else
                            If .Rows(intR).Item("kind") = "1" Or .Rows(intR).Item("kind") = "4" Then   '收
                                If .Rows(intR).Item("cotn_code") = "A" Then   '應收數(保留)
