...
Public Class ACBK110
    Dim confid, confunit As String, LoadAfter, Dirty As Boolean
    Dim sqlstr As String
    Dim strAccno As String
    Dim SYear, EYear As Integer
    Dim SDate, EDate As String
    Dim intCol As Integer = 5   '宣告欄位數為6欄 (1個收入欄及5個費用別)  假若要更動欄位數則要更改intcol & grid.ColumnStyles(n).Width
    '宣告陣列最大值=9 方便日後程式更動欄位數 because can't dim 變數 array
    Dim AryAccno(9) As String  '宣告每件工程之帳簿有1個收入欄及9個費用別 accno科目代號
    Dim Aryaccname(9) As String  '宣告每件工程之帳簿有1個收入欄及9個費用別 accname科目名稱
    Dim AryBgAmt(9) As Decimal   '宣告每件工程之帳簿有1個收入欄及9個費用別 預算數 
    Dim AryAcuamt(9) As Integer   '定義各欄年累計數
    Dim AryMonamt(9) As Integer   '定義各欄月累計數
    Dim mydataset, myds, mydsD, mydsAccno As DataSet
    Dim PageRow As Integer = 30  '每頁印28行

    Private Sub ACBK110_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        '找過帳日期
        Dim sqlstr As String
        ...
        End If
        vxtSAccno.Text = "13701"    '起值
        vxtEAccno.Text = "137019"   '迄值
    End Sub
    ...
    Private Sub BtnPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnPrint.Click
        ...
        Dim document As New FPDocument("列印未完工程補助計畫明細帳")
        ...
        For intD = 0 To mydsD.Tables("DataAccno").Rows.Count - 1
            ...
            sqlstr = "SELECT c.accno, c.accname," & sqlstr2 & strBg & " FROM " & _
                     ...
                     " LEFT OUTER JOIN bgf010 b ON b.ACCNO = c.ACCNO " & _
                     " ORDER BY  c.ACCNO"
            ...
            sqlstr = "SELECT a.dc, sum(a.amt) as amt FROM " & _
                     " (select * from acf020 where left(other_accno,16)='" & strAccno & "') a " & _
            ...
            intR = 0     '該7級科目所有傳票資料
            '找acf020傳票資料並要併入捐贈公積3-1102 and other_accno=七該級科目 (from acf020)資料(table body record )
            sqlstr = "SELECT a.accyear, a.accno, b.date_2 as date_2, a.kind, a.no_2_no, a.seq, " & _
                     "a.item, a.remark, a.amt, a.cotn_code, a.dc, a.other_accno FROM " & _
                     " (select * from acf020 where accyear>=" & SYear & " and left(accno,16)='" & strAccno & "') a " & _
                     "left outer join acf010 b on a.accyear=b.accyear and a.kind=b.kind " & _
                     "and a.no_2_no=b.no_2_no and a.seq=b.seq and b.item='1' " & _
                     "where b.date_2 between '" & SDate & "' and '" & EDate & "'" & _
                     " union " & _
                     "SELECT c.accyear, c.other_accno as accno, d.date_2 as date_2, c.kind, " & _
                     "c.no_2_no, c.seq, c.item, c.remark, c.amt, c.cotn_code, c.dc, c.other_accno FROM " & _
                     " (select * from acf020 where accyear>=" & SYear & " and left(other_accno,16)='" & _
                      strAccno & "' and accno='31102') c " & _
                     "left outer join acf010 d on c.accyear=d.accyear and c.kind=d.kind " & _
                     "and c.no_2_no=d.no_2_no and c.seq=d.seq and d.item='1'" & _
                     " where d.date_2 between '" & SDate & "' and '" & EDate & _
            ...
            For PageCnt As Integer = 1 To 999    '頁次
                ...
                If blnIsBelong Then
                    ...
                    '定欄位寬tot324
                    grid.ColumnStyles(1).Width = 14    '假若要更動欄位數則要更改 
                    grid.ColumnStyles(2).Width = 6
                    grid.ColumnStyles(3).Width = 12
                    grid.ColumnStyles(4).Width = 100
                    For intI = 5 To intCol + 7
                        grid.ColumnStyles(intI).Width = 24
                    ...
                    grid.Cells2D(1, 5 + intCol + 1).RowSpan = 2
                    grid.Cells2D(1, 5 + intCol + 2).RowSpan = 2
                    ...
                    grid.Texts2D(1, 5 + intCol + 1).Text = "合       計 ."
                    grid.Texts2D(1, 5 + intCol + 2).Text = "餘       額 ."
                ...
                If intR = 0 Then   '上月累計
                    If AryAcuamt(0) + AryAcuamt(1) + AryAcuamt(2) + AryAcuamt(3) + AryAcuamt(4) + AryAcuamt(5) + _
                       AryAcuamt(6) + AryAcuamt(7) + AryAcuamt(8) + AryAcuamt(9) <> 0 Then
                        i += 1
                        ...
                        For intI = 0 To intCol
                            ...
                            If intI > 0 Then AcuTot = AcuTot + AryAcuamt(intI) '支出合計
                        Next
                        grid.Texts2D(i, 5 + intCol + 1).Text = Format(AcuTot, "###,###,###,###.##")
                        grid.Texts2D(i, 5 + intCol + 2).Text = Format(AryAcuamt(0) - AcuTot, "###,###,###,###.##")   '餘額
                    End If
                    MonTot = 0
                    ...
                    Do While i < PageRow
                        If intR > .Rows.Count - 1 Then   'end of file 
                            If MonTot <> 0 Then
                                ...
                                MonTot = 0
                                For intI = 0 To intCol
                                    grid.Texts2D(i, intI + 5).Text = Format(AryMonamt(intI), "###,###,###,###.##")  '自第五欄起放array
                                    If intI > 0 Then MonTot = MonTot + AryMonamt(intI) '支出合計
                                Next
                                grid.Texts2D(i, 5 + intCol + 1).Text = Format(MonTot, "###,###,###,###.##")
                            ...
                            If AcuTot <> 0 Then
                                ...
                                Next
                                grid.Texts2D(i, 5 + intCol + 1).Text = Format(AcuTot, "###,###,###,###.##")
                                grid.Texts2D(i, 5 + intCol + 2).Text = Format(AryAcuamt(0) - AcuTot, "###,###,###,###.##")
                                '必須清0,否則當此行為最後一行時,次頁會再印
                                AcuTot = 0
                            End If
                            PageCnt = 1000
                        ...
                        blnControlEnd = False   '是否結束轉帳
                        If intI = 0 Then '收入欄
                            If strDC = "1" Then intAmt = -intAmt '3-1102 at debit is 減少
                            If Not (Not IsDBNull(.Rows(intR).Item("other_accno")) AndAlso Mid(.Rows(intR).Item("other_accno"), 1, 5) = "13701") Then
                                If strDC = "2" Then
                                    intAmt = -intAmt '1-3701 at credit is 結束轉帳
                                    blnControlEnd = True
                                End If
                            End If
                        Else             '支出欄
                            If strDC = "2" Then intAmt = -intAmt '1-3701 at credit is 減少
                        End If
                        If blnControlEnd = True Then   '結束轉帳只印金額,不加入月計累計
                            grid.Texts2D(i, 5).Text = Format(intAmt, "###,###,###,###.##")
                        Else
