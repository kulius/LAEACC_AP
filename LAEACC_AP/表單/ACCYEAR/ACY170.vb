...
Public Class ACY170
    ...
    Private Sub LoadGridFunc()
        ...
        sqlstr += ",(case when a.cramt12 is null then 0 else a.cramt12 end)-"
        sqlstr += "(case when a.deamt12 is null then 0 else a.deamt12 end)-(case when e.amt6 is null then 0 else e.amt6 end) as amt5 "
        ...
        sqlstr += "(select * from accname where len(accno)=9 and substring(accno,1,1)='4') c "
        sqlstr += "left outer join "
        sqlstr += "(select accno, deamt12, cramt12 from acf050 where "
        sqlstr += "accyear=" & SYear & " and len(accno)=9 and substring(accno,1,1)='4') a "
        ...
        sqlstr += "from accbg where accyear=" & SYear & " and len(accno)=9 and substring(accno,1,1)='4') d "
        sqlstr += "on c.accno=d.accno "
        sqlstr += "left outer join "
        sqlstr += "(select b.accno, sum(b.amt) as amt6 from "
        sqlstr += "(select left(accno,9) as accno, case dc when 1 then amt*-1 else amt end as amt "
        sqlstr += "from acf020 where accyear=" & SYear & " and left(accno,1)='4' and cotn_code='A' and no_2_no > 0) b "
        sqlstr += "group by b.accno) e "
        sqlstr += "on c.accno=e.accno "    'sqlstr 99/11/11原五級修改至六級
        retstr = runsql(mastconn, sqlstr)
        If retstr <> "sqlok" Then MsgBox("insert acm010 error " & vbNewLine & sqlstr)
        ...
        sqlstr += ",case when e.amt6 is null then 0 else e.amt6 end as amt6 "
        sqlstr += ",case when d.amt7 is null then 0 else d.amt7 end as amt7 from "
        sqlstr += "(select * from accname where len(accno)=7 and substring(accno,1,1)='4') c "
        sqlstr += "left outer join "
        sqlstr += "(select accno, deamt12, cramt12 from acf050 where "
        sqlstr += "accyear=" & SYear & " and len(accno)=7 and substring(accno,1,1)='4') a "
        sqlstr += "on c.accno=a.accno "
        sqlstr += "left outer join "
        sqlstr += "(select accno, (bg1+bg2+bg3+bg4+bg5) as amt3,(up1+up2+up3+up4+up5) as amt4, (bg1+bg2+bg3+bg4+bg5+up1+up2+up3+up4+up5) as amt7 "
        sqlstr += "from accbg where accyear=" & SYear & " and len(accno)=7 and substring(accno,1,1)='4') d "
        sqlstr += "on c.accno=d.accno "
        sqlstr += "left outer join "
        sqlstr += "(select b.accno, sum(b.amt) as amt6 from "
        sqlstr += "(select left(accno,7) as accno, case dc when 1 then amt*-1 else amt end as amt "
        sqlstr += "from acf020 where accyear=" & SYear & " and left(accno,1)='4' and cotn_code='A' and no_2_no > 0) b "
        sqlstr += "group by b.accno) e "
    ...
    Private Sub BtnPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnPrint.Click
        ...
        Try
            ...
            Try
                tt1 = "c:\App\acc\報表樣本\ACY170.xls"
                tt2 = "c:\App\acc\報表\ACY170.xls"
                If Not File.Exists(tt1) Then
                    AppReport_Copy("acc", "ACY170.xls", tt1)
            ...
            Catch ex As Exception
                MsgBox("報表copy from " & tt1 & "  to  " & tt2 & " 錯誤，若 \報表\ACY170.XLS 使用中請先關閉之，否則請洽資訊人員!")
            ...
            With myds.Tables("acm010")
                '第一列是合計資料
                If .Rows(0).Item("accno") = "4" Then
                    TOT1 = nz(.Rows(0).Item("amt5"), 0)  '本年度實收入總額
                    TOT2 = nz(.Rows(0).Item("amt6"), 0)  '上年度權責收入總額
                    TOT3 = nz(.Rows(0).Item("amt7"), 0)  '本年度收入預算總額
