...
Public Class ACBK040
    ...
    Private Sub ACBK040_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        '找過帳日期
        Dim sqlstr As String
    ...
    Private Sub BtnPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnPrint.Click
        ...
        sqlstr = "SELECT a.accno, c.accname, d.accname as name4, e.accname as name5, " & sqlstr2 & " FROM ACF050 a " & _
                 ...
                 " FROM (SELECT accno FROM acf050 WHERE accyear =" & SYear & " AND LEN(ACCNO) <> 17 AND " & _
                 " accno BETWEEN '" & Saccno & "' AND '" & Eaccno & "') b " & _
                 " WHERE len(a.accno) < 10 and a.accno = LEFT(b.accno, len(a.accno)) AND a.accno <> b.accno)) AND (LEN(a.ACCNO) <> 17 AND LEFT(a.ACCNO,5)>'11103' and " & _
                  " left(a.accno,3)<>'113' and left(a.accno,3)<>'151' and left(a.accno,3)<>'212' and left(a.accno,3)<>'114') " & _
                " ORDER BY  a.ACCNO"
        mydsD = openmember("", "DataAccno", sqlstr)
        'mydsD = CHIA.RDA.gRDA.OpenMember("", "DataAccno", sqlstr)   '自阿傑chia.rda元件取資料 有300秒
        For intD = 0 To mydsD.Tables("DataAccno").Rows.Count - 1
            ...
            strName4 = nz(mydsD.Tables("DataAccno").Rows(intD).Item("name4"), "") '列印科目
            ...
            Else
                ...
                End If
            End If
            If rdoBelongCNo.Checked Then   '已是補助工程者不印
                ...
                If mydataset.Tables("acf020").Rows.Count > 0 Then
                    If nz(mydataset.Tables("acf020").Rows(0).Item("belong"), "") = "C" Then GoTo NEXTFOR
                End If
            End If
            ...
            For PageCnt As Integer = 1 To 999    '頁次
                ...
                With mydataset.Tables("acf020")
                    Do While i < PageRow
                        If intR > .Rows.Count - 1 Then   '最後一筆
                            '99/11/16 update
                            If strEnd = "Y" Then
                                If TransPara.TransP("UnitTitle").indexof("嘉南") >= 0 Then
                                    If strAccno = "32301    I" Or strAccno = "32301    00" Then
                                        sqlstr = "select * from acy020 where accno='" & strAccno & "'"
                                        myds = openmember("", "acf020", sqlstr)
                                        If myds.Tables(0).Rows.Count > 0 Then
                                            i += 1
                                            If i > PageRow Then Exit Do
                                            sAmt = nz(myds.Tables(0).Rows(0).Item("amt5"), 0)
                                            If sAmt > 0 Then   '收入-支出
                                                grid.Texts2D(i, 7).Text = "貸"
                                                grid.Texts2D(i, 6).Text = FormatNumber(sAmt, 2)
                                                AcuCramt += sAmt
                                                MonCramt += sAmt
                                            Else
                                                grid.Texts2D(i, 7).Text = "借"
                                                grid.Texts2D(i, 5).Text = FormatNumber(-sAmt, 2)
                                                AcuDeamt += -sAmt
                                                MonDeamt += -sAmt
                                            End If
                                            grid.Texts2D(i, 4).Text = "結清各項收支轉入本期餘絀"
                                            grid.Texts2D(i, 1).Text = ShortDate(EDate)
                                            grid.Texts2D(i, 2).Text = "轉"
                                            grid.Texts2D(i, 3).Text = Format(intEndNo - 1, "#####")
                                        End If
                                    End If
                                Else  '非嘉南
                                    If strAccno = "32301" Then '計算本期損益
                                        sqlstr = "select sum(cramt12)-sum(deamt12) as net from acf050 where accyear=" & _
                                                 SYear & " and left(accno,1)>='4' AND left(accno,1)<='5' and len(accno)=5"
                                        myds = openmember("", "acf050", sqlstr)
                                        If myds.Tables(0).Rows.Count > 0 And nz(myds.Tables(0).Rows(0).Item("net"), 0) <> 0 Then
                                            i += 1
                                            If i > PageRow Then Exit Do
                                            sAmt = nz(myds.Tables(0).Rows(0).Item("net"), 0) '本期損益
                                            If sAmt > 0 Then   '收入-支出
                                                grid.Texts2D(i, 7).Text = "貸"
                                                grid.Texts2D(i, 6).Text = FormatNumber(sAmt, 2)
                                                AcuCramt += sAmt
