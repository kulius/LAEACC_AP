...
Public Class ACY212
    ...
    Private Sub ACY212_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub LoadGridFunc()
        ...
        sqlstr = "select a.*, b.accname as accname from (select * from acf060 where accyear=" & _
                  SYear & " and left(accno,3)='212' and len(accno)>=5 and len(accno)<=16) a " & _
    ...
    Private Sub BtnPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnPrint.Click
        ...
        Try
            ...
            Try
                tt1 = "c:\App\acc\報表樣本\ACY212.xls"
                tt2 = "c:\App\acc\報表\ACY212.xls"
                If Not File.Exists(tt1) Then
                    AppReport_Copy("acc", "ACY212.xls", tt1)
            ...
            Catch ex As Exception
                MsgBox("報表copy from " & tt1 & "  to  " & tt2 & " 錯誤，若 \報表\ACY212.XLS 使用中請先關閉之，否則請洽資訊人員!")
            ...
            With myds.Tables("acm010")
                For intR = 0 To .Rows.Count - 1
                    ...
                    NAR(xlRange)
                    xlRange = xlCells(i, 4)
                    xlRange.Value = FormatNumber(.Rows(intR).Item("account12") - (.Rows(intR).Item("beg_credit") - .Rows(intR).Item("beg_debit")), 2)
                    NAR(xlRange)
                    xlRange = xlCells(i, 5)
                    ...
                    If Len(strAccno) = 5 Then '由四級作合計
                        ...
                        TOT4 += .Rows(intR).Item("account12") - (.Rows(intR).Item("beg_credit") - .Rows(intR).Item("beg_debit"))
                        TOT5 += .Rows(intR).Item("ACT12")
                        TOT6 += .Rows(intR).Item("sub12")
                        TOT7 += .Rows(intR).Item("cramt12") - .Rows(intR).Item("deamt12")
                    End If
                Next
            End With
