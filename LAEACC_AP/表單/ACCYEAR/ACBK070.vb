...
Public Class ACBK070
    ...
    Private Sub ACBK070_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        '找過帳日期
        Dim sqlstr As String
        ...
        End If
        vxtSAccno.Text = "11201"    '起值
        vxtEAccno.Text = "112019"   '迄值
    End Sub
    ...
    Private Sub BtnPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnPrint.Click
        ...
        Dim document As New FPDocument("列印有價證券明細分戶帳")
        ...
        Dim ggrid As New FPTable(0, 17, 324, 222, PageRow, 10)
        '宣告一變數儲存是否符合頁碼範圍
        ...
        sqlstr = "SELECT a.accno, c.accname, g.kind, g.unit, " & sqlstr2 & " FROM ACF050 a " & _
        ...
        For intD = 0 To mydsD.Tables("DataAccno").Rows.Count - 1
            ...
            For PageCnt As Integer = 1 To 999    '頁次
                ...
                If blnIsBelong Then
                    ...
                    textTitle = New FPText("有價證券明細分戶帳", 90, 6)
                    ...
                    textName4 = New FPText("會計科目：" & FormatAccno(strAccno), 0, 6)
                    textName7 = New FPText("資產名稱：" & strName7 & "  計算單位：" & strUnit, 0, 12)
                    grid = New FPTable(0, 17, 324, 222, PageRow, 10)   '新增表格物件x,y,w,h,row,col (單位是公厘)
                    grid.Font.Size = 10
                    ...
                    grid.ColumnStyles(4).Width = 145
                    grid.ColumnStyles(5).Width = 20
                    grid.ColumnStyles(6).Width = 20
                    grid.ColumnStyles(7).Width = 10
                    grid.ColumnStyles(8).Width = 36
                    grid.ColumnStyles(9).Width = 36
                    ...
                    grid.Texts2D(1, 5).Text = "證券號碼"
                    grid.Texts2D(1, 6).Text = "數      量  ."
                    grid.Texts2D(1, 7).Text = "單位"
                    grid.Texts2D(2, 8).Text = "面          額  ."
                    grid.Texts2D(2, 9).Text = "總　　    　額  ."
                    grid.Texts2D(1, 8).Text = "金                              額           ."
                    grid.Texts2D(1, 10).Text = "備註"
                Else
                    textUnit = ttextUnit
                ...
                If intR = 0 Then   '上月累計
                    If AcuDeamt <> 0 Or AcuCramt <> 0 Then
                        ...
                        grid.Texts2D(i, 6).Text = Format(AcuDeQty - AcuCrQty, "###,###,###,###.######")
                        grid.Texts2D(i, 8).Text = Format(rate(AcuDeamt - AcuCramt, (AcuDeQty - AcuCrQty) * 100, 2), "###,###,###.##")
                        grid.Texts2D(i, 9).Text = Format(AcuDeamt - AcuCramt, "###,###,###,##0.00")
                    End If
                End If
                    ...
                    Do While i < PageRow
                        If intR > .Rows.Count - 1 Then
                            ...
                            If strEnd = "Y" And AcuDeamt <> AcuCramt Then
                                ...
                                grid.Texts2D(i, 9).Text = FormatNumber(AcuDeamt - AcuCramt, 2)
                        ...
                        If .Rows(intR).Item("dc") = "1" Then   '借方
                            ...
                            grid.Texts2D(i, 8).Text = Format(rate(.Rows(intR).Item("amt"), .Rows(intR).Item("mat_qty") * 100, 2), "###,###,###.##")
                            grid.Texts2D(i, 9).Text = Format(.Rows(intR).Item("amt"), "###,###,###,##0.00")
