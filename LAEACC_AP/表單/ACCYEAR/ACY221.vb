...
Public Class ACY221
    ...
    Private Sub ACY221_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub LoadGridFunc()
        ...
        '丟入dataset 
        sqlstr = "select a.*, b.accname as accname,c.deamt,c.cramt, d.loaner from (select * from acf050 where accyear=" & _
                  SYear & " and left(accno,3)='221' and len(accno)>=5 and len(accno)<=16) a " & _
                 " left outer join accname b on a.accno=b.accno " & _
                 " left outer join accbg c on a.accno=c.accno and a.accyear=c.accyear " & _
                 " left outer join acy22101 d on a.accno=d.accno " & _
                 " order by a.accno"
    ...
    Private Sub BtnPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnPrint.Click
        ...
        Try
            ...
            Dim TOT3, TOT4, TOT5, TOT7, TOT8, TOT10 As Decimal
            TOT3 = 0 : TOT4 = 0 : TOT5 = 0 : TOT7 = 0 : TOT8 = 0 : TOT10 = 0

            Dim tt1, tt2 As String
            Try
                tt1 = "c:\App\acc\報表樣本\ACY221.xls"
                tt2 = "c:\App\acc\報表\ACY221.xls"
                If Not File.Exists(tt1) Then
                    AppReport_Copy("acc", "ACY221.xls", tt1)
            ...
            Catch ex As Exception
                MsgBox("報表copy from " & tt1 & "  to  " & tt2 & " 錯誤，若 \報表\ACY221.XLS 使用中請先關閉之，否則請洽資訊人員!")
            ...
            With myds.Tables("acm010")
                For intR = 0 To .Rows.Count - 1
                    ...
                    xlRange = xlCells(i, 1)
                    xlRange.Value = nz(.Rows(intR).Item("loaner"), "")  '債權機關
                    NAR(xlRange)
                    xlRange = xlCells(i, 2)
                    xlRange.Value = Space(spaces) & FormatAccno(strAccno) & vbCrLf & Space(spaces) & strAccname
                    NAR(xlRange)
                    xlRange = xlCells(i, 3)  '上年度
                    xlRange.Value = FormatNumber(.Rows(intR).Item("beg_credit") - .Rows(intR).Item("beg_debit"), 2)
                    NAR(xlRange)
                    xlRange = xlCells(i, 4)   '決算舉借
                    xlRange.Value = FormatNumber(.Rows(intR).Item("cramt12") - .Rows(intR).Item("beg_credit"), 2)
                    NAR(xlRange)
                    xlRange = xlCells(i, 5)   '預算舉借
                    xlRange.Value = FormatNumber(nz(.Rows(intR).Item("cramt"), 0), 2)
                    NAR(xlRange)
                    xlRange = xlCells(i, 6)   '舉借之比較增減數
                    xlRange.Value = FormatNumber(.Rows(intR).Item("cramt12") - .Rows(intR).Item("beg_credit") - nz(.Rows(intR).Item("cramt"), 0), 2)
                    NAR(xlRange)
                    xlRange = xlCells(i, 7)   '決算償還
                    xlRange.Value = FormatNumber(.Rows(intR).Item("deamt12") - .Rows(intR).Item("beg_debit"), 2)
                    NAR(xlRange)
                    xlRange = xlCells(i, 8)    ''預算償還
                    xlRange.Value = FormatNumber(nz(.Rows(intR).Item("deamt"), 0), 2)
                    NAR(xlRange)
                    xlRange = xlCells(i, 9)    '償還之比較增減數
                    xlRange.Value = FormatNumber(.Rows(intR).Item("deamt12") - .Rows(intR).Item("beg_debit") - nz(.Rows(intR).Item("deamt"), 0), 2)
                    NAR(xlRange)
                    xlRange = xlCells(i, 10)    ''決算餘額
                    ...
                    If Len(strAccno) = 5 Then '由四級作合計
                        TOT3 += .Rows(intR).Item("beg_credit") - .Rows(intR).Item("beg_debit")
                        TOT4 += .Rows(intR).Item("cramt12") - .Rows(intR).Item("beg_credit")
                        TOT5 += nz(.Rows(intR).Item("cramt"), 0)
                        TOT7 += .Rows(intR).Item("deamt12") - .Rows(intR).Item("beg_debit")
                        TOT8 += nz(.Rows(intR).Item("deamt"), 0)
                        TOT10 += .Rows(intR).Item("cramt12") - .Rows(intR).Item("deamt12")
                    End If
                Next
            End With
            ...
            NAR(xlRange)
            xlRange = xlCells(i, 6)
            xlRange.Value = FormatNumber(TOT4 - TOT5, 2)
            NAR(xlRange)
            xlRange = xlCells(i, 7)
