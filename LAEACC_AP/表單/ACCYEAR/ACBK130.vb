...
Public Class ACBK130
    Dim confid, confunit As String, LoadAfter, Dirty As Boolean
    Dim sqlstr As String
    Dim strAccno As String
    Dim strRaccno As String  '收入科目
    ...
    Private Sub ACBK130_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        '找過帳日期
        Dim sqlstr As String
        ...
        End If
        vxtRAccno.Text = "4140201"   '收入科目
        vxtSAccno.Text = "5290101"    '起值
        vxtEAccno.Text = "52901029"  '迄值
    End Sub
    ...
    Private Sub BtnPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnPrint.Click
        ...
        strRaccno = GetAccno(vxtRAccno.Text)             '收入科目
        ...
        Dim document As New FPDocument("列印補助計畫明細帳(費用類)")
        ...
        document.SetDefaultPageMargin(30, 0, 10, 10)     'left,top,right,botton
        ...
        For intD = 0 To mydsD.Tables("DataAccno").Rows.Count - 1
            ...
            sqlstr = "SELECT c.accno, c.accname," & sqlstr2 & strBg & " FROM " & _
                     ...
                     " LEFT OUTER JOIN bgf010 b ON a.ACCNO = b.ACCNO " & _
                     " ORDER BY  c.ACCNO"
            ...
            sqlstr = "SELECT a.dc, sum(a.amt) as amt FROM " & _
                     " (select * from acf020 where left(other_accno,16)='" & strAccno & "' and accno='" & strRaccno & "') a " & _
            ...
            intR = 0     '該7級科目所有傳票資料
            '找acf020傳票資料並要併入科目4-1402-01 and other_accno=七該級科目 (from acf020)資料(table body record )
            sqlstr = "SELECT a.accyear, a.accno, b.date_2 as date_2, a.kind, a.no_2_no, a.seq, " & _
                     ...
                     " c.no_2_no, c.seq, c.item, c.remark, c.amt, c.cotn_code, c.dc FROM " & _
                     " (select * from acf020 where accyear>=" & SYear & " and left(other_accno,16)='" & _
                      strAccno & "' and accno='" & strRaccno & "') c " & _
            ...
            For PageCnt As Integer = 1 To 999    '頁次
                ...
                With mydataset.Tables("acf020")
                    Do While i < PageRow
                        ...
                        If intI = 0 Then '收入欄
                            If strDC = "1" Then intAmt = -intAmt '4-1402-01 at debit is 減少 
                        Else             '支出欄
                            If strDC = "2" Then intAmt = -intAmt '5-2901 at credit is 減少
                        End If
