...
Public Class ACY24
    ...
    Private Sub ACY24_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub LoadGridFunc()
        ...
        sqlstr = "select a.*, b.accname as accname,c.deamt,c.cramt from (select * from acf050 where accyear=" & _
                  SYear & " and left(accno,2)='24' and len(accno)>=5 and len(accno)<=9 ) a " & _
    ...
    Private Sub BtnPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnPrint.Click
        ...
        Try
            ...
            Dim TOT2, TOT3, TOT4, TOT6, TOT7, TOT9 As Decimal
            TOT2 = 0 : TOT3 = 0 : TOT4 = 0 : TOT6 = 0 : TOT7 = 0 : TOT9 = 0

            Dim tt1, tt2 As String
            Try
                tt1 = "c:\App\acc\報表樣本\ACY24.xls"
                tt2 = "c:\App\acc\報表\ACY24.xls"
                If Not File.Exists(tt1) Then
                    AppReport_Copy("acc", "ACY24.xls", tt1)
            ...
            Catch ex As Exception
                MsgBox("報表copy from " & tt1 & "  to  " & tt2 & " 錯誤，若 \報表\ACY24.XLS 使用中請先關閉之，否則請洽資訊人員!")
            ...
            With myds.Tables("acm010")
                For intR = 0 To .Rows.Count - 1
                    ...
                    If Len(strAccno) = 5 Then '由四級作合計
                        TOT2 += .Rows(intR).Item("beg_credit") - .Rows(intR).Item("beg_debit")
                        TOT3 += .Rows(intR).Item("cramt12") - .Rows(intR).Item("beg_credit")
                        TOT4 += nz(.Rows(intR).Item("cramt"), 0)
                        TOT6 += .Rows(intR).Item("deamt12") - .Rows(intR).Item("beg_debit")
                        TOT7 += nz(.Rows(intR).Item("deamt"), 0)
                        TOT9 += .Rows(intR).Item("cramt12") - .Rows(intR).Item("deamt12")
                    End If
                Next
            End With
