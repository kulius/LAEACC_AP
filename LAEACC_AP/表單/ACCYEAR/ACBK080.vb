...
Public Class ACBK080
    ...
    Private Sub ACBK080_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        '找過帳日期
        Dim sqlstr As String
        ...
        End If
        vxtSAccno.Text = "13101"    '起值
        vxtEAccno.Text = "136019"   '迄值
    End Sub
    ...
    Private Sub BtnPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnPrint.Click
        ...
        Dim document As New FPDocument("列印固定資產明細分戶帳")
        ...
        sqlstr = "SELECT a.accno, c.accname, d.accname as name5, g.kind, g.unit, " & sqlstr2 & " FROM ACF050 a " & _
                 ...
                 " left(a.accno,5)<>'13201' "
        If rdoAccno2No.Checked Then sqlstr += " and substring(a.accno,5,1)<>'2' "
        sqlstr += " and (a.ACCNO NOT IN (SELECT LEFT(b.accno, len(a.accno)) AS accno " & _
                 " FROM (SELECT accno FROM acf050 WHERE accyear =" & SYear & "AND left(accno,5)<>'13201' "
        If rdoAccno2No.Checked Then sqlstr += " and substring(accno,5,1)<>'2' "
        sqlstr += " and accno BETWEEN '" & Saccno & "' AND '" & Eaccno & "') b " & _
                 " WHERE a.accno = LEFT(b.accno, len(a.accno)) AND a.accno <> b.accno)) " & _
                 " ORDER BY  a.ACCNO"
        '        "修正前為WHERE len(a.accno) < 10 and a.accno = LEFT(b.accno, len(a.accno)) AND a.accno <> b.accno)) " & _
        ...
        For intD = 0 To mydsD.Tables("DataAccno").Rows.Count - 1
            ...
            '上月累計or上年度轉入
            If Mid(strAccno, 5, 1) = "1" Then  '資產科目
                AcuDeamt = nz(mydsD.Tables("DataAccno").Rows(intD).Item("deamt"), 0)
                AcuCramt = nz(mydsD.Tables("DataAccno").Rows(intD).Item("cramt"), 0)
            Else     '累計折舊科目
                AcuDeamt = nz(mydsD.Tables("DataAccno").Rows(intD).Item("cramt"), 0)
                AcuCramt = nz(mydsD.Tables("DataAccno").Rows(intD).Item("deamt"), 0)
            End If
            ...
            For PageCnt As Integer = 1 To 999    '頁次
                ...
                If blnIsBelong Then
                    ...
                    textTitle = New FPText("固定資產明細分戶帳", 90, 6)
                    ...
                    textName4 = New FPText("總帳科目：" & FormatAccno(strAccno) & "  " & strName5, 0, 6)
                    textName7 = New FPText("資產名稱：" & strName7 & "  規格：" & strKind & "  計算單位：" & strUnit, 0, 12)
                    ...
                    grid.ColumnStyles(4).Width = 130
                    grid.ColumnStyles(5).Width = 36
                    grid.ColumnStyles(6).Width = 36
                    grid.ColumnStyles(7).Width = 37
                    grid.ColumnStyles(8).Width = 20
                    grid.ColumnStyles(9).Width = 13

                    grid.Cells2D(1, 1).ColSpan = 3 '12,3 隱藏
                    grid.Cells2D(1, 6).ColSpan = 2
                    ...
                    grid.Texts2D(1, 5).Text = "數          量  ."
                    grid.Texts2D(2, 6).Text = "單          價  ."
                    grid.Texts2D(2, 7).Text = "金　　    　額  ."
                    grid.Texts2D(1, 8).Text = "存放處所"
                    grid.Texts2D(1, 9).Text = "備  註"
                Else
                    textUnit = ttextUnit
                ...
                If intR = 0 Then   '上月累計
                    If AcuDeamt <> 0 Or AcuCramt <> 0 Then
                        ...
                        grid.Texts2D(i, 5).Text = Format(AcuDeQty - AcuCrQty, "###,###,###,###.######")
                        grid.Texts2D(i, 6).Text = Format(rate(AcuDeamt - AcuCramt, (AcuDeQty - AcuCrQty) * 100, 2), "###,###,###.##")
                        grid.Texts2D(i, 7).Text = Format(AcuDeamt - AcuCramt, "###,###,###,##0.00")
                    End If
                End If
                    ...
                    Do While i < PageRow
                        If intR > .Rows.Count - 1 Then
                            If MonDeamt + MonCramt <> 0 Then
                                ...
                                strRemark = "　　　　本　　月　　合　　計"
                                grid.Texts2D(i, 4).Text = strRemark
                                grid.Texts2D(i, 5).Text = Format(MonDeQty - MonCrQty, "###,###,###,###.######")
                                grid.Texts2D(i, 6).Text = Format(rate(MonDeamt - MonCramt, (MonDeQty - MonCrQty) * 100, 2), "###,###,###.##")
                                grid.Texts2D(i, 7).Text = Format(MonDeamt - MonCramt, "###,###,###,##0.00")
                                MonDeamt = 0 : MonCramt = 0 : MonDeQty = 0 : MonCrQty = 0
                            End If
                            ...
                            If strEnd = "Y" And AcuDeamt <> AcuCramt Then
                                ...
                                grid.Texts2D(i, 5).Text = Format(AcuDeQty - AcuCrQty, "###,###,###,###.######")
                                grid.Texts2D(i, 6).Text = Format(rate(AcuDeamt - AcuCramt, (AcuDeQty - AcuCrQty) * 100, 2), "###,###,###.##")
                                grid.Texts2D(i, 7).Text = FormatNumber(AcuDeamt - AcuCramt, 2)
                        ...
                        If Month(.Rows(intR).Item("date_2")) <> tempMonth Then  '日期不同時要印小計
                            If MonDeamt + MonCramt <> 0 Then
                                ...
                                grid.Texts2D(i, 4).Text = strRemark
                                grid.Texts2D(i, 5).Text = Format(MonDeQty - MonCrQty, "###,###,###,###.######")
                                grid.Texts2D(i, 6).Text = Format(rate(MonDeamt - MonCramt, (AcuDeQty - AcuCrQty) * 100, 2), "###,###,###.##")
                                grid.Texts2D(i, 7).Text = Format(MonDeamt - MonCramt, "###,###,###,##0.00")
                                MonDeamt = 0 : MonCramt = 0 : MonDeQty = 0 : MonCrQty = 0
                            End If
                            If AcuDeamt + AcuCramt <> 0 Then
                        ...
                        If (.Rows(intR).Item("dc") = "1" And Mid(strAccno, 5, 1) = "1") Or (.Rows(intR).Item("dc") = "2" And Mid(strAccno, 5, 1) = "2") Then '借方
                            grid.Texts2D(i, 5).Text = Format(.Rows(intR).Item("mat_qty"), "###,###,###,###.######")
                            grid.Texts2D(i, 6).Text = Format(rate(.Rows(intR).Item("amt"), .Rows(intR).Item("mat_qty") * 100, 2), "###,###,###.##")
                            grid.Texts2D(i, 7).Text = Format(.Rows(intR).Item("amt"), "###,###,###,##0.00")
